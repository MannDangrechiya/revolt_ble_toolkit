...
- The pipeline fully materializes a capture's packets into memory (`list(...)` in `run_pipeline`) rather than streaming end to end. **Measured, not assumed**, against a synthetic reproducible 174.7 KiB / 4,706-packet capture (see RELEASE_AUDIT.md for why this replaced the previous real-device measurement): full pipeline 72–122 ms across 3 runs, peak memory a stable 3.11 MiB; linearly extrapolated to ~12 minutes / ~18 GiB at 1 GiB, the scale where this would start to matter. See KNOWN_LIMITATIONS.md for the full profiling breakdown. Documented rather than rewritten — see REMAINING_TASKS.md for why.
