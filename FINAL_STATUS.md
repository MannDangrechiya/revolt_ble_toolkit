...
| Desktop GUI | 90% (core features complete; some Qt paint/mouse-event glue untestable without a display — not a functional gap) |
...
- Desktop GUI: drag-and-drop, search, category filters, click-to-seek timeline, hex/ASCII/statistics panes, dark theme.
...
Full detail in `ROADMAP.md` and `REMAINING_TASKS.md`. Highest-value next step is **not more code** — it's capturing the RV400 in more real states (charging, different ride modes, GPS in motion) to validate and improve heuristic accuracy. After that: PCAP export, GUI parity for export/compare, a streaming pipeline (only once a capture actually demonstrates the need).

## Production Readiness

- **Tests:** 120 passing, 0 failing, 0 skipped.
- **Coverage:** 91% line coverage overall.
